    ...
    public static class ClientScene
    {
        ...
        public static void PrepareToSpawnSceneObjects()
        {
            ...
            foreach (NetworkIdentity identity in allIdentities)
            {
                ...
                {
					//Debug.Log("Adding spawnable " + identity.gameObject.name + " " + identity.netId);
        ...
        internal static bool FindOrSpawnObject(SpawnMessage msg, out NetworkIdentity identity)
        {
            ...
            if (identity == null)
            {
                logger.LogError($"Could not spawn assetId={msg.assetId} scene={msg.sceneId:X} netId={msg.netId} pos={msg.position}");
                return false;
            }
			//Debug.Log($"Spawned assetId={msg.assetId} scene={msg.sceneId:X} netId={msg.netId} pos={msg.position}");
